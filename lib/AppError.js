...
export const ERROR_CODES = {
  ...
  // A passage marked in a book that the book no longer contains — almost always
  // because it was re-extracted between the page loading and the marker being
  // dragged. Specific because the user can act on it, and the action is not the
  // obvious one: reload and mark again, rather than retry the same request.
  ...
  // A file that cannot be DISPLAYED, which is a different limit from the one
  // above and needs a different sentence. .doc can be summarised by nobody and
  // shown by nobody, but the two refusals reach the user on different screens —
  // and "cannot be summarised" beside a blank reader explains nothing.
  UNVIEWABLE_TEXT_FORMAT: "UNVIEWABLE_TEXT_FORMAT",
  // A file of a format we CAN show, that this particular copy of cannot be read
  // — corrupt, truncated, password-protected, or simply not what its extension
  // claims. Distinct from the code above because the advice is opposite: there
  // is nothing to re-save, the file itself is the problem.
